...
class TestVidCloudTasks(TestCase):
    def test_vid_cloud_tasks(self):
        HOSTNAME = 'thinking-glass-282301.uc.r.appspot.com'
        sendurl = 'https://' + HOSTNAME + '/guests/4/3/qlHxCHcgJo/post_vid/'
        with requests.Session() as s:
            r = s.post(sendurl, data='guest1'.encode('utf-8'), verify=False)
            content = json.loads(r.content)
            secureurl = content['url']
            # print(secureurl)
            print('content vid', content['vid'])
            successurl = 'https://' + HOSTNAME + '/guests/4/3/qlHxCHcgJo/'+str(content['vid'])+'/'
            print(successurl)
